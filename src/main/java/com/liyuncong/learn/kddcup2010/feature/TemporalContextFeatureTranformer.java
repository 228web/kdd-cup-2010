 ...
 *
 */
public class TemporalContextFeatureTranformer implements Serializable{
	private static final long serialVersionUID = -499548205482995752L;
	private transient Logger logger = LoggerFactory.getLogger(TemporalContextFeatureTranformer.class);
	...
	public TemporalContextFeatureTranformer(HiveContext hiveContext, JavaSparkContext sparkContext, DataFrame trainData,
	...
	/**
	 * 联合训练集和测试集，一起添加temporalFeature；
	 * Test rows are determined by a program that randomly selects one problem for 
	 * each student within a unit, and places all student-step rows for that 
	 * student and problem in the test file. Based on time, all preceding 
	 * student-step rows for the unit will be placed in a training file, while all 
	 * following student-step rows for that unit will be discarded. 
	 * @param data
	 * @return
	 */
	private DataFrame temporalFeature(DataFrame data) {
		data.select("anonStudentId", "problemName", "stepName", "kcDefault");
